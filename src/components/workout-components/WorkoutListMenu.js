...
function WorkoutListMenu (props) {
    ...
    return (
        <div className="workout-list-menu">
            ...
            <Button className="blue-button" id="createWorkoutBtn" title="createWorkoutBtn" sx={{ borderRadius: 1, minWidth: 30, minHeight: 0, padding: 0.8, margin: 0.5, backgroundColor: "#00008b", color: "#ffffff", '&:hover': { backgroundColor: "#4040A8" } }} onClick={() => {
                props.createFunc();
                ...
                {(typeof workoutList !== 'undefined') && workoutList.map((workout, i) => {
                    return <Grid item key={"workout" + i.toString()} xs={12} sx={{ width: 1 }}>
                        <Card variant="outlined" sx={{ margin: 0.5, padding: 0.5, border: 3, borderRadius: 2, borderColor: selectedWorkout === workout.workout_id ? '#00008b' : '#E8E8F5', backgroundColor: '#E8E8F5', color: "#00008b" }}>
                            ...
                            <Button id="viewDetailsBtn" sx={{ borderRadius: 1, minWidth: 30, minHeight: 0, padding: 0.8, margin: 0.5, backgroundColor: "#00008b", color: "#ffffff", '&:hover': { backgroundColor: "#4040A8" } }} onClick={() => {
                                setSelectedWorkout(null);
                                props.selectFunc(null);
                                props.viewFunc(workout.workout_id);
                            }}>View Details</Button>
                            <Button id="assignBtn" sx={{ borderRadius: 1, minWidth: 30, minHeight: 0, padding: 0.8, margin: 0.5, backgroundColor: "#00008b", color: "#ffffff", '&:hover': { backgroundColor: "#4040A8" } }} onClick={() => {
                            ...
                            <Button id="editBtn" sx={{ borderRadius: 1, minWidth: 30, minHeight: 0, padding: 0.8, margin: 0.5, backgroundColor: "#00008b", color: "#ffffff", '&:hover': { backgroundColor: "#4040A8" } }} onClick={() => {
                                setSelectedWorkout(null);
                                props.selectFunc(null);
                                props.editFunc(workout.workout_id);
                            }}>Edit</Button>
                            <Button id="deleteBtn" sx={{ borderRadius: 1, minWidth: 30, minHeight: 0, padding: 0.8, margin: 0.5, backgroundColor: "#8C0044", color: "#ffffff", '&:hover': { backgroundColor: "#A94073" } }} onClick={() => {
                                setSelectedWorkout(null);
                                props.selectFunc(null);
